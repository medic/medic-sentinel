...
const getClinicID = doc => {
  return getClinic(doc)
    .then(clinic => clinic && clinic._id);
};

const getParent = (facility, type) => {
  return new Promise((resolve, reject) => {
    var id = facility && facility._id;
    if (!id) {
      return resolve();
    }
    db.medic.view('medic-client', 'docs_by_id_lineage', {
      ...
      var parent = results.rows.find(row => row.doc.type === type);
      resolve(parent && parent.doc);
    });
  });
};
...
const getClinic = doc => getParent(doc.contact, 'clinic');
const getHealthCenter = doc => getParent(doc.contact, 'health_center');
const getDistrict = doc => getParent(doc.contact, 'district_hospital');

const getHealthCenterPhone = doc => {
  return getHealthCenter(doc)
    .then(getContact)
    .then(contact => contact && contact.phone);
};

const getDistrictPhone = doc => {
  return getDistrict(doc)
    .then(getContact)
    .then(contact => contact && contact.phone);
};
...
const getRecentForm = (options, callback) => {
  ...
  getClinicID(options.doc).then(clinicId => {
    if (!clinicId) {
...
module.exports = {
  ...
  getClinicPhone: doc => {
    return getClinic(doc)
      .then(getContact)
      .then(clinicContact => {
        const clinicPhone = clinicContact && clinicContact.phone;
        if (clinicPhone) {
          return clinicPhone;
        }
        return getContact(doc)
          .then(docContact => docContact && docContact.phone);
      });
  },
  getClinicName: (doc, noDefault) => {
    return getClinic(doc)
      .then(clinic => {
        const name = (clinic && clinic.name) || (doc && doc.name);
        if (name || noDefault) {
          return name;
        }
  ...
  getClinicContactName: (doc, noDefault) => {
    return getClinic(doc)
      .then(getContact)
      .then(clinicContact => {
        let name = clinicContact && clinicContact.name;
        if (name) {
          return name;
        }
        return getContact(doc)
          .then(docContact => {
            name = docContact && docContact.name;
            if (name || noDefault) {
              return name;
            }
            return 'health volunteer';
