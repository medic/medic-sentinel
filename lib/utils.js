...
var getMaxSMS = function() {
  return config && config.get('max_sms') || 10;
};

var getMaxSMSLength = function(text) {
  var max = getMaxSMS();
  
  if ( isGSMAlphabet(text) ) {
    return max == 1 ? 160 : max * 153;
  }
  else {
    return max == 1 ? 70 : max * 63;
  }
};

var numberOfSMS = function(text) {
  var n;
  if ( isGSMAlphabet(text) ) {
    n = text.length <= 160 ? 1 : Math.ceil(text.length/153);
  }
  else {
    n = text.length <= 70 ? 1 : Math.ceil(text.length/67);
  }
  return n;
};

var isGSMAlphabet = function(text) {
  var regexp = new RegExp("^[A-Za-z0-9 \\r\\n@£$¥èéùìòÇØøÅå\u0394_\u03A6\u0393\u039B\u03A9\u03A0\u03A8\u03A3\u0398\u039EÆæßÉ!\"#$%&'()*+,\\-./:;<=>?¡ÄÖÑÜ§¿äöñüà^{}\\\\\\[~\\]|\u20AC]*$");

  return regexp.test(text);
};

var truncateMessage = function(text) {
  var maxLength = getMaxSMSLength(text);
...
var prepareSMS = function(text) {
  var sms = {};
  sms.message = truncateMessage(text);
  sms.info = {
    number_of_sms: numberOfSMS(sms.message),
    is_truncated: sms.message != text
  };
  if ( sms.info.is_truncated ) {
    sms.info.original = {
      message: text,
      number_of_sms: numberOfSMS(text)
    };
  }
  return sms;
};
...
module.exports = {
    ...
    getMaxSMS: getMaxSMS,
    getMaxSMSLength: getMaxSMSLength,
    numberOfSMS: numberOfSMS,
    isGSMAlphabet: isGSMAlphabet,
