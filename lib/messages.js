...
function getMessage(configuration, locale) {
    ...
    // otherwise, use the configured messages
    var messages = configuration.messages || configuration.message;
    if (!_.isArray(messages)) {
        console.warn('Message property should be an array. Please check that your configuration is valid.');
        return '';
    }
    if (!messages.length) {
        console.warn('Message property array way empty. Please check that your configuration.');
        return '';
    }
