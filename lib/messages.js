...
function addMessage(opts) {
    ...
    if (opts.phone) {
        phonePromise = Promise.resolve(opts.phone);
    } else {
        phonePromise = module.exports.getRecipientPhone(doc, 'clinic');
    }

    return phonePromise.then(phone => {
        if (phone) {
            try {
...
module.exports = {
    ...
    getRecipientPhone: function(doc, recipient, _default) {
        return new Promise((resolve, reject) => {
            ...
            _default = _default || doc.from;
            recipient = recipient && recipient.trim();
            if (!recipient) {
                return resolve(_default);
            }
            var ret = doc.fields && doc.fields[recipient];
            if (recipient === 'reporting_unit') {
                ret = doc.from;
            } else {
                let promise;
                if (recipient === 'clinic') {
                    promise = utils.getClinicPhone(doc);
                } else if (recipient === 'parent') {
                    promise = utils.getHealthCenterPhone(doc);
                } else if (recipient === 'grandparent') {
                    promise = utils.getDistrictPhone(doc);
                }
                if (promise) {
                    return promise
                        .then(ret => {
                            resolve(ret || _default);
                        })
                        .catch(reject);
                }
            }
