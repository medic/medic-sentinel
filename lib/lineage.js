...
const fillContactsInDocs = (docs, contacts) => {
  ...
  contacts.forEach(contactDoc => {
    // FIXME: because we use find here, if there is more than one doc that
    //       needs this contact we only bind it to the first one
...
const fetchHydratedDoc = id => {
  return new Promise((resolve, reject) => {
    db.medic.view('medic-client', 'docs_by_id_lineage', {
      ...
      // FIXME: We do not unique this before getting it from the DB
